...
// ─── Bug Fix #1: generateSampleData mutated the same date object in nested loops.
// Fixed by creating a fresh Date per entry instead of calling setHours on a shared ref.
const generateSampleData = (dailyGoal = 2500) => {
  ...
  for (let i = 14; i >= 0; i--) {
    ...
    for (let j = 0; j < entries; j++) {
      ...
      // Bug Fix #1 continued: create a fresh Date for each entry timestamp
...
// ─── Bug Fix #2: getInitialState now populates sample logs and a default profile
// so the app is functional from the start (no more blank stats/calendar screens).
const getInitialState = () => {
  ...
  return {
    profile: null, // null = onboarding not done yet
    logs: [],
...
export default function WaterIntakeTracker() {
  ...
  // ─── Onboarding state (Bug Fix #3: was defined but had no corresponding UI)
  ...
  const addWater = (amount) => {
    ...
    setData(prev => {
      ...
      // ─── Bug Fix #4: celebration check used logs[todayIndex].total AFTER mutation.
      // Fixed by capturing prevTotal before building the new log object.
  ...
  // ─── Bug Fix #5: streaks memo now guards against null profile safely.
  const streaks = useMemo(() => {
    ...
    for (let i = 0; i < sorted.length; i++) {
      ...
      else if (i === 0) break; // today didn't meet goal
      else break;
    }
  ...
  // ─────────────────────────────────────────────
  // Bug Fix #3: Full onboarding UI (was missing entirely)
  // ─────────────────────────────────────────────
  function OnboardingView() {
    ...
    return (
      ...
      >
        ...
        {onboardingStep === 1 && (
          <div className="space-y-4">
            ...
            <input
              ...
              className="w-full bg-white/10 border border-white/20 rounded-2xl p-4 text-white text-lg focus:outline-none focus:ring-2 focus:ring-cyan-500"
              autoFocus
            />
            <div>
              ...
              <input
                type="number"
                value={local.weight}
                onChange={e => setLocal(p => ({ ...p, weight: parseInt(e.target.value) || 70 }))}
                className="w-full bg-white/10 border border-white/20 rounded-2xl p-4 text-white focus:outline-none focus:ring-2 focus:ring-cyan-500"
              />
            </div>
          </div>
        )}
        ...
        {onboardingStep === 2 && (
          <div className="space-y-4">
            <h2 className="text-2xl font-bold text-white">How active are you?</h2>
        ...
        {onboardingStep === 3 && (
          <div className="space-y-4">
            <h2 className="text-2xl font-bold text-white">What's your climate like?</h2>
            <div className="grid grid-cols-1 gap-3">
              {[
        ...
        <motion.button
          ...
          onClick={handleNext}
          disabled={!canNext()}
          className={`w-full rounded-2xl p-4 font-semibold text-white mt-4 transition-opacity ${
            canNext()
      ...
      >
        ...
        <div className="relative flex items-center justify-center py-8">
          <svg className="w-64 h-64 transform -rotate-90">
            <circle cx="128" cy="128" r="110" fill="none" stroke="rgba(255,255,255,0.05)" strokeWidth="16" />
            <motion.circle
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.5, type: 'spring' }} className="text-center">
              ...
              <div className="text-gray-400 mt-2 text-sm">{todayTotal}ml / {dailyGoal}ml</div>
              <div className="text-cyan-400 mt-1 text-xs font-medium">{remaining}ml remaining</div>
            </motion.div>
          </div>
        </div>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <h3 className="font-semibold text-white">{getMotivation()}</h3>
              <p className="text-gray-400 text-sm mt-1">{streaks.current} day streak</p>
            </div>
          </div>
        </motion.div>
        ...
        <div className="space-y-3">
          <h3 className="text-sm font-medium text-gray-400 uppercase tracking-wide">Quick Add</h3>
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              >
                ...
                <div className="text-sm font-medium text-white">{amount}ml</div>
          ...
          <motion.button
            ...
            className="w-full bg-gradient-to-r from-cyan-500 to-blue-500 rounded-2xl p-4 font-semibold text-white shadow-lg shadow-cyan-500/25 hover:shadow-cyan-500/40 transition-shadow flex items-center justify-center gap-2"
          >
        ...
        {todayLog.entries.length > 0 && (
          <div className="space-y-3">
            <h3 className="text-sm font-medium text-gray-400 uppercase tracking-wide">Today's Log</h3>
            <div className="space-y-2">
              {[...todayLog.entries].reverse().slice(0, 5).map(entry => (
                <motion.div
                  key={entry.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
                  className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-4 flex items-center justify-between"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-semibold text-white">{entry.amount}ml</div>
  ...
  // ─────────────────────────────────────────────
  // STATS VIEW — Bug Fix #6: bestDay crash when logs empty; fixed with safe fallback
  // ─────────────────────────────────────────────
  function StatsView() {
    // Bug Fix #7: pass data via closure correctly; logs slice must guard against empty
    ...
    const stats = useMemo(() => {
      ...
      // Bug Fix #6: safe reduce with initial value guard
    ...
    return (
      ...
      >
        <div>
          ...
          <p className="text-gray-400 mt-1">Your hydration insights</p>
        </div>

        <div className="grid grid-cols-2 gap-3">
          <StatCard icon={<TrendingUp className="w-5 h-5" />} label="Avg Daily" value={`${stats.avgIntake}ml`} color="from-cyan-400 to-cyan-500" />
          <StatCard icon={<Target className="w-5 h-5" />} label="Goals Hit" value={`${stats.goalsAchieved}/7`} color="from-blue-400 to-blue-500" />
          <StatCard icon={<Flame className="w-5 h-5" />} label="Current Streak" value={`${streaks.current} days`} color="from-orange-400 to-orange-500" />
          <StatCard icon={<Award className="w-5 h-5" />} label="Best Streak" value={`${streaks.best} days`} color="from-purple-400 to-purple-500" />
        </div>
        ...
        {/* Bug Fix #6: only render bestDay card when data exists */}
        {stats.bestDay && (
          <div className="bg-gradient-to-br from-yellow-500/10 to-orange-500/10 backdrop-blur-sm border border-yellow-500/20 rounded-3xl p-6">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <h3 className="font-semibold text-white">Best Day This Week</h3>
  ...
  // ─────────────────────────────────────────────
  // CALENDAR VIEW — Bug Fix #8: date parsing timezone issue fixed with T12:00:00
  // ─────────────────────────────────────────────
            ...
            {monthData.map((day, idx) => (
              <motion.div
                ...
                className={`aspect-square rounded-xl flex items-center justify-center text-sm font-medium ${
                  !day ? 'bg-transparent'
                  : day.achieved ? 'bg-gradient-to-br from-cyan-500 to-blue-500 text-white shadow-lg shadow-cyan-500/25'
                  : day.total > 0 ? 'bg-white/10 text-gray-300'
                  : day.isToday ? 'bg-white/10 text-cyan-400 ring-1 ring-cyan-400/50'
    ...
    return (
      ...
      >
          ...
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-3xl p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <div className="text-sm text-gray-400 mt-1">Target intake per day</div>
              </div>
              <Target className="w-8 h-8 text-cyan-400" />
            </div>
            <motion.button
              whileTap={{ scale: 0.98 }} onClick={() => setShowGoalCalculator(true)}
              className="w-full bg-cyan-500/20 border border-cyan-500/30 rounded-xl p-3 font-medium text-cyan-400 hover:bg-cyan-500/30 transition-colors flex items-center justify-center gap-2"
            >
          ...
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-3xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <span className="font-medium text-white">Enable Reminders</span>
              </div>
              <button
                onClick={() => setData(prev => ({ ...prev, reminders: { ...prev.reminders, enabled: !prev.reminders.enabled } }))}
                className={`w-12 h-7 rounded-full transition-colors ${data.reminders.enabled ? 'bg-cyan-500' : 'bg-white/20'}`}
              >
                <motion.div animate={{ x: data.reminders.enabled ? 20 : 0 }} className="w-5 h-5 bg-white rounded-full m-1" />
              </button>
            </div>
            <div>
              ...
              <input
                ...
                className="w-full bg-white/10 border border-white/20 rounded-xl p-3 text-white focus:outline-none focus:ring-2 focus:ring-cyan-500"
              />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="bg-gradient-to-br from-orange-500/10 to-red-500/10 backdrop-blur-sm border border-orange-500/20 rounded-3xl p-6">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <div className="text-sm text-gray-400">Total Days Tracked</div>
                <div className="text-3xl font-bold text-white mt-1">{streaks.total}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <motion.button
          ...
          className="w-full bg-red-500/10 border border-red-500/20 rounded-2xl p-3 font-medium text-red-400 hover:bg-red-500/20 transition-colors"
        >
  ...
  // ─────────────────────────────────────────────
  // GOAL CALCULATOR MODAL — Bug Fix #9: uses safe defaults so calculateGoal never crashes
  // ─────────────────────────────────────────────
    ...
    return (
      ...
      >
        <motion.div
          ...
          className="bg-gradient-to-br from-gray-900 to-gray-800 border border-white/10 rounded-3xl p-6 w-full max-w-md max-h-[80vh] overflow-y-auto"
        >
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-2xl font-bold text-white">Calculate Goal</h3>
            <button onClick={() => setShowGoalCalculator(false)} className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center hover:bg-white/20 transition-colors">
          ...
          <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-2">
                {['low', 'moderate', 'high'].map(level => (
                  <button key={level} onClick={() => setLocalProfile(p => ({ ...p, activityLevel: level }))}
                    className={`p-3 rounded-xl font-medium transition-colors ${localProfile.activityLevel === level ? 'bg-cyan-500 text-white' : 'bg-white/10 text-gray-400 hover:bg-white/20'}`}>
              ...
              <div className="grid grid-cols-3 gap-2">
                {['cool', 'moderate', 'hot'].map(c => (
                  <button key={c} onClick={() => setLocalProfile(p => ({ ...p, climate: c }))}
                    className={`p-3 rounded-xl font-medium transition-colors ${localProfile.climate === c ? 'bg-blue-500 text-white' : 'bg-white/10 text-gray-400 hover:bg-white/20'}`}>
            ...
            <div className="bg-gradient-to-br from-cyan-500/20 to-blue-500/20 border border-cyan-500/30 rounded-2xl p-6 mt-6">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-cyan-950 text-white relative overflow-hidden">
      {/* Animated Background */}
      ...
      <div className="relative z-10 max-w-lg mx-auto px-6 py-8">
        <AnimatePresence mode="wait">
          {/* Bug Fix #3: Render onboarding when profile not set */}
          {needsOnboarding ? (
